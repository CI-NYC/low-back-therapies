...
anxiety_icds <- read_csv(file.path(home_dir, "data/public/anxiety_icd10_20230323.csv"), col_names = F) |>
...
all_anxiety_clean <- 
  ...
  mutate(anxiety_washout_cal = case_when(min_anxiety_dt_0 %within% interval(washout_start_dt, day0_dt - 1) ~ 1,
                                              TRUE ~ 0)) |>
  select(BENE_ID, min_anxiety_dt_0, 
         anxiety_washout_cal)

write_data(all_anxiety_clean, "anxiety.rds", file.path(drv_root_12_month_washout, "baseline_covariates")) # save final data file