...
overall_1 <- table_one_function(data |> filter(subset_oud %in% c(0,1)))[1:59,]
oud_no_1 <- table_one_function(data |> filter(subset_oud == 0))[1:59,]
oud_yes_1 <- table_one_function(data |> filter(subset_oud == 1))[1:59,2]

overall_2 <- table_one_function(data |> filter(subset_oud %in% c(0,1)))[60:91,]
oud_no_2 <- table_one_function(data |> filter(subset_oud == 0))[60:91,]
...
oud_no_2_7day_gap <- table_one_function(data_7day_gap |> filter(subset_oud == 0))[60:79,]
...
part2_7day_gap <- cbind(oud_no_2_7day_gap)
