...
all_bipolar_clean <- 
  ...
  mutate(bipolar_post_exposure_cal = case_when(min_bipolar_dt_0 %within% interval(pain_diagnosis_dt, exposure_end_dt) ~ 1,
                                         TRUE ~ 0)) |>
  select(BENE_ID, min_bipolar_dt_0,
         bipolar_post_exposure_cal)

write_data(all_bipolar_clean, "bipolar_post_exposure.rds", file.path(drv_root, "baseline_covariates")) # save final data file