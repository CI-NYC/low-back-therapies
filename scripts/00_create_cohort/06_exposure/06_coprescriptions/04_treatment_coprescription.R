# -------------------------------------
# Script: 02_mediator_coprescription.R
# Author: Nick Williams
# Updated:
# Purpose: Creates variables for whether or not an observation in
...
# RXL datasets
opioid <- load_data("exposure_period_opioids.fst", file.path(drv_root, "treatments"))

rxl_claims <- open_rxl() |>
  filter(CLM_ID %in% opioid$CLM_ID) |>
  select(CLM_ID) |>
  collect()

opioid <- opioid |>
  filter(CLM_ID %in% rxl_claims$CLM_ID) |>
  select(BENE_ID, RX_FILL_DT = rx_start_dt, DAYS_SUPPLY = days_supply) |>
  as.data.table()

benzo <- readRDS(file.path(drv_root, "treatments/treatment_benzo_rx.rds"))
gaba <- readRDS(file.path(drv_root, "treatments/treatment_gaba_rx.rds"))
mrelax <- readRDS(file.path(drv_root, "treatments/treatment_mrelax_rx.rds"))
...
prop_days_covered <- function(data) {
  ...
  min(max(time_length(dur, "days") / 91, 1 / 91), 1)
}
...
opioids_benzos <- 
  ...
  mutate(exposure_opioid_benzo_copresc = 1, 
         exposure_prop_opioid_benzo_copresc = mediator_prop_opioid_benzo_copresc) |> 
  select(-data)
...
opioids_gabas <- 
  ...
  mutate(exposure_opioid_gaba_copresc = 1, 
         exposure_prop_opioid_gaba_copresc = mediator_prop_opioid_gaba_copresc) |> 
  select(-data)
...
opioids_mrelax <- 
  ...
  mutate(exposure_opioid_mrelax_copresc = 1, 
         exposure_prop_opioid_mrelax_copresc = mediator_prop_opioid_mrelax_copresc) |> 
  select(-data)
...
walk(list(opioids_benzos, opioids_gabas, opioids_mrelax), 
     setDT)
...
saveRDS(coprescriptions, file.path(drv_root, "treatments/opioid_coprescriptions.rds"))
