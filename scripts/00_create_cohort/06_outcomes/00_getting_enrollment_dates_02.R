...
washout <- load_data("pain_washout_continuous_enrollment_with_exposures.fst", file.path(drv_root, "treatment")) |>
...
for (i in seq_along(files)) { 
  ...
  dt[, c(paste0("period_", 1:num_periods, "_end_dt")) := 
       lapply(1:num_periods, function(p) exposure_period_end_dt + days(p * follow_up_period_length))]
