...
cohort <- load_data("pain_washout_continuous_enrollment_with_exposures.fst", file.path(drv_root, "treatment")) |>
  mutate(followup_start_dt = exposure_period_end_dt + days(1),
         study_end_dt = exposure_period_end_dt + days(366)) |>
  select(BENE_ID, followup_start_dt, study_end_dt)
...
rxl_opioids <- 
  ...
  filter((RX_FILL_DT >= followup_start_dt) & 
           (RX_FILL_DT <= study_end_dt))

otl_opioids <- 
  otl_opioids |> 
  filter((LINE_SRVC_BGN_DT >= followup_start_dt) & 
           (LINE_SRVC_BGN_DT <= study_end_dt))
...
write_data(opioids_wo_moud_bup, "opioid_dts_12mos.fst", file.path(drv_root, "treatment"))