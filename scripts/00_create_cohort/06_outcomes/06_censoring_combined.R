...
for (i in c("")){
  ...
  cohort <- load_data(paste0("pain_washout_continuous_enrollment_with_exposures", i, ".fst"), file.path(drv_root, "treatment"))
  ...
  cohort <- fmutate(
    cohort, 
    period_1 = interval(
      first_treatment_dt, exposure_period_end_dt + days(91) # exposure period + period 1
    ), 
    period_2 = interval(
  ...
  dual_cens <- 
    ...
    filter(elig_dt %within% interval(first_treatment_dt, exposure_period_end_dt + days(455))) |> 
    group_by(BENE_ID) |>
    arrange(elig_dt) |> 
    filter(row_number() == 1) |> 
    ungroup() |> 
  ...
  dual_codes2 <- 
    ...
    fselect(BENE_ID, washout_start_dt, first_treatment_dt, code, elig_dt) |> 
