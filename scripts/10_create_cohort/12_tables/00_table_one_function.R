...
table_one_function <- function(df){
  ...
  df <- df |>
    ...
    mutate(
      ...
      dem_race_white = ifelse(dem_race == "White, non-Hispanic", 1, 0),
      dem_race_white = ifelse(missing_dem_race == 0, dem_race_white, NA),
      ...
      oth_20_washout_cal_bin = as.numeric(num_oth_washout_cal > 0 & num_oth_washout_cal <= 20),
      oth_40_washout_cal_bin = as.numeric(num_oth_washout_cal > 20),
  ...
  demographics <- c("\\textbf{Baseline covariates (months -6 to 0)}",
                    ...
                    "Outpatient visits",
                    paste0("\\hspace{0.5cm}", c("0", "1-20", "21+")),
                    # "Prescriptions",
