...
low_back_analysis_df <- readRDS(file.path(low_back_dir, "low_back_analysis_df.rds"))
...
clean <- copy(low_back_analysis_df)
...
clean[, `:=`(dem_sex_m = fifelse(dem_sex == "M", 1, 0), # Sex (reference category set to female)
             ...
             dem_race = fifelse(dem_race == "White, non-Hispanic", 1, 0),
...
saveRDS(clean, file.path(low_back_dir, "low_back_df_for_table_one.rds"))
