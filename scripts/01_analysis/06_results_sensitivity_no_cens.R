...
version <- "sensitivity" # sensitivity analysis using 30-day allowable gaps
...
relabel <- function(data) {
  mutate(data, 
         treatment = case_when(
           ...
           treatment == "exposure_duloxetine" ~ "Duloxetine", 
...
ragg::agg_png(
  glue("~/medicaid/low-back-therapies/figures/{Y}/7day_mtp_{Y}_outcome_fix_n_oud_relrisk_no_cens.png"), 
  width = 7, height = 3.5, units = "cm", res = 600
)
