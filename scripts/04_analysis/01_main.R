...
drv_root <- "/mnt/general-data/disability/low-back-therapies/final"
data <- readRDS(file.path(drv_root, "low_back_analysis_df_clean.rds"))
...
# options: ["oud_18mo", "oud_24mo"]
Y <- args[[4]]
# options: [2, 5]
folds <- as.numeric(args[[5]])

# use <- data[disability_pain_cal == subset]
use <- copy(data)
setDF(use)
...
saveRDS(fit, file.path(drv_root, "mtp_fits", 
                       glue("fit_{gsub(' ', '_', subset)}_{Y}_outcome_fix_mediator_{M[[1]][mediator]}.rds")))