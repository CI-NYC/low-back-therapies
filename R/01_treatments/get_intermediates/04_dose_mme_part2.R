...
rxl_opioids <- read_rds(file.path(data_dir, "rxl_opioids_pain_mme.rds")) |> filter(BENE_ID %in% dts_cohorts$BENE_ID)
otl_opioids <- read_rds(file.path(data_dir, "otl_opioids_pain_mme.rds")) |> filter(BENE_ID %in% dts_cohorts$BENE_ID)
...
saveRDS(all_opioids_clean, file.path(low_back_dir, "treatments/all_pain_opioids.rds"))
