...
low_back_dir <- "/mnt/general-data/disability/low-back-therapies/treatments"
...
mediator_df[, mediator_has_multimodal_pain_treatment_restrict := fcase(
  mediator_has_minimally_invasive_spinal_procedure == 1, 1,
...
acupuncture <- readRDS(file.path(drv_root, "mediator_has_acupuncture.rds"))
blocks <- readRDS(file.path(drv_root, "mediator_has_blocks.rds"))
chiropractic <- readRDS(file.path(drv_root, "mediator_has_chiropractic.rds"))
epidural_steroid <- readRDS(file.path(drv_root, "mediator_has_epidural_steroid.rds"))
massage_therapy <- readRDS(file.path(drv_root, "mediator_has_massage_therapy.rds"))
trigger_point_injection <- readRDS(file.path(drv_root, "mediator_has_trigger_point_injection.rds"))

mediator_df <- mediator_df |>
  # select(BENE_ID, mediator_has_multimodal_pain_treatment_restrict) |>
  left_join(acupuncture) |>
  left_join(blocks) |>
  left_join(chiropractic) |>
  left_join(epidural_steroid) |>
  left_join(massage_therapy) |>
  left_join(trigger_point_injection)
  
saveRDS(mediator_df, 
        file.path(low_back_dir, "mediator_has_multimodal_pain_treatment_restrict.rds"))
