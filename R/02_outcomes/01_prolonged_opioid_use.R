...
opioids <- otl |> 
  mutate(rx_int = interval(LINE_SRVC_BGN_DT, LINE_SRVC_BGN_DT + days(1)), 
         rx_int = intersect(rx_int, interval(trt_end_dt + 1, 
                                             followup_12mos_dt))) |> 
...
opioids$outcome_opioid_days_supply <- 
...
opioids <- select(dts_cohorts, BENE_ID) |> 
  ...
  mutate(outcome_opioid_days_supply = replace_na(outcome_opioid_days_supply, 0),
         prolonged_opioid_use = ifelse(outcome_opioid_days_supply>=120, 1, 0))

saveRDS(opioids, file.path(low_back_dir, "outcomes/prolonged_opioid_use.rds"))